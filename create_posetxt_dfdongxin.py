...
if __name__ == '__main__':
    data_list = ['WOMEN', 'MEN']
    data_root = '/data/Datasets/deepfashion_dongxin/img_highres_op'
    ...
    # pose_train_txt = open('/data/Datasets/DiOR/DeepFashionDX/fasion-annotation-train.csv', 'w')
    # pose_train_txt.write('name:keypoints_y:keypoints_x\n')
    pose_test_txt = open('/data/Datasets/DiOR/DeepFashionDX/fasion-annotation-test.csv', 'w')
    pose_test_txt.write('name:keypoints_y:keypoints_x\n')
    im_name_list = []

    for data in data_list:
        data_path = os.path.join(data_root, data)
        for attr_dir in tqdm.tqdm(os.listdir(data_path)):
            attr_dir_path = os.path.join(data_path, attr_dir)
            for pose_dir in os.listdir(attr_dir_path):
                pose_dir_path = os.path.join(attr_dir_path, pose_dir)
                for pose_name in os.listdir(pose_dir_path):
                    ...
                    im_name = data + '-' + attr_dir + '-' + pose_dir + '-' + pose_name[:-4]
